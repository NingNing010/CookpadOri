...
interface RecipeCarousel2Props {
    user_id?: number;
}

const RecipeCarousel2 = ({ user_id = 0 }: RecipeCarousel2Props) => {
    const { data: postsData } = useQuery({
        queryKey: [QUERY_KEY.Recipes.GET_LIST_RECICPE + "following"],
        queryFn: () => ApiHome.getUserRecipe(user_id, 0, 10),
        staleTime: 1000 * 60 * 5,
    });
...
export default RecipeCarousel2;
